...
1. Run Docker container with MQTT broker (use Docker desktop or `docker run -p 8080:8080 -p 1883:1883 hivemq/hivemq4` in terminal)
3. On another terminal, run `ssh -R 1883:<laptop ipv4 addr>:1883 xilinx@172.26.191.218`. This sets up the reverse ssh tunelling from the Ultra96 to the MQTT broker. 
...
1. Download `eval_server.zip` from telegram. 
...
### On Ultra96 Laptop
1. On a terminal, run `ssh -R 8888:<laptop ipv4 addr>:<server port> xilinx@172.26.191.218`. This sets up the reverse ssh tunelling from the Ultra96 to the MQTT broker.  
